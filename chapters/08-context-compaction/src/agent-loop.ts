/**
 * 07 章基线（快照，runtime 不变）：06 章增量 Context Transformation + runAgentLoopContinue。
 *
 * 08 章不修改本文件：Session Compaction 不在 loop 内实现。
 * 层次（08 章起生效）：
 *   Session Tree → buildSessionContext（08 章重建投影）→ canonical state.messages
 *   → transformContext（06 章钩子，extension 注入接缝）→ call-time messages
 * loop 只消费 canonical state，对 compaction 零感知（Pi：compaction 在
 * AgentSession/SessionManager 层，低层 loop 只有通用 transformContext 选项）。
 *
 * 相对 05 章的变化（全部锚定 Pi）：
 ...
 * emitContext）；Pi coding-agent 的 compaction 不使用 shouldStopAfterTurn。
 */
