/**
 * 07 章基线：最小 Agent 封装（Reduce it——精读 Pi `Agent` class（agent.ts L173）后的最小复刻）。
 * 12 章组装复用：本文件与 08 章对应文件保持一致（含 replaceMessages seam，Product Assembly 的 Agent 层同步副本）。
 *
 * 复刻的设计（全部锚定 Pi）：
 ...
 *
 * 08 章新增（本文件唯一增量）：
 * - `replaceMessages`：用重建后的 canonical context 替换状态消息。
 *   Pi 的对应写法是 agent-session.ts L2004-2007/L2329-2332：
 *   agent.state.messages = buildSessionContext().messages（compact 成功后当场刷新，
 *   同一进程内的下一次 run 无需 reload session 即使用 summary + 保留段）。
 */
...
export class Agent {
  ...
  /**
   * 08 章 seam：用重建后的 canonical context 替换状态消息
   * （Pi agent-session.ts L2004-2007：agent.state.messages = buildSessionContext().messages）。
   * compact 成功后由调用方以 compactSession(...).context 调用；
   * 下一次 prompt/continue 即使用 summary + 保留段，无需 reload session。
   */
